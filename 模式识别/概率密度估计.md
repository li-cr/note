...
- 首先：在给定样本集D的情况下，估计关于参数的后验分布$\,p(\mu|D)\,$。
	...
	- $\large\begin{aligned} p(\mu|D) &=\alpha\prod\limits_{i=1}^np(x_i|\mu)p(\mu) \\ &=\alpha\prod\limits_{i=1}^{n} A\exp\Big(-\frac{1}{2}\frac{(x_i-\mu)^2}{\sigma^2}\Big)B\exp\Big(-\frac{1}{2}\frac{(\mu-\mu_0)^2}{\sigma_0^2}\end{aligned}$