...
- $P(D)是与参数无关的归一化因子，根据全概率公式:p(D=\sum\limits_{i}p(D|\theta_i)p(\theta_i)$
	...
	- 可得到贝叶斯参数估计的<font color="#00ffff">后验概率密度函数</font>：
 	...
 	> - $\huge p(\theta|D) =\frac{p(D|\theta)p(\theta)}{\int_{\theta}p(D|\theta)p(\theta)d\theta}=\frac{\prod\limits{i=1}^n{p(x_i|\theta)p(\theta)}}{\int_\theta\prod\limits{i=1}^{n}{p(x_i|\theta)}p(\theta)d\theta} = \alpha\prod\limits_{i=1}^np(x_i|\theta)p(\theta)$
