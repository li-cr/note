...
- <font color="#ff30">后验数据分布</font>
	- 但现在获得了有关$\theta$的后验估计$p(\theta|D)，如何估计p(x|D)？$考虑<font color="#a0ffff">全概率公式和边际分布：</font>
		- $\begin{aligned}p(x|D)&=\int_\theta p(x,\theta|D)d\theta\\&=\int_\theta\frac{p(x,\theta,D)}{p(D)}d\theta\\ &=\int_\theta\frac{p(x|\theta,D)p(\theta,D)}{p(D)}d\theta \\&=\int_\theta p(x|\theta,D)p(\theta|D)d\theta\\ &=\int_\theta p(x|\theta)p(\theta|D) \end{aligned}$