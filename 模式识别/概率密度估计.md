...
- <font color="#ffa000">贝叶斯学习中的迭代计算公式</font>
	...
	- 有以下迭代公式：
		- $\begin{aligned}p(\theta|D^n)&=\frac{p(D^n|\theta)p(\theta)}{\int p(D^n|\theta)p(\theta)d\theta}=\frac{p(x_n|\theta)p(D^{n-1}|\theta)p(\theta)}{\int p(x_n|\theta)p(D^{n-1}|\theta)p(\theta)d\theta}\\ &=\frac{p(x_n|\theta)}{\int p(x_n|\theta)\frac{p(D^{n-1}|\theta)p(\theta)d\theta} \cdot \frac{p(D^{n-1}|\theta)p(\theta)}{\int p(D^{n-1}|\theta)p(\theta)d\theta} \end{aligned}$