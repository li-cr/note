...
- 首先：在给定样本集D的情况下，估计关于参数的后验分布$\,p(\mu|D)\,$。
	...
	- $\displaystyle p(\mu|D)\sim N(\mu_n,\sigma_n^2)=\frac{1}{\sqrt{2\pi\sigma_n}}\exp\Bigg(-\frac{1}{2}\frac{(\mu-\mu_n)^2}{\sigma_n^2}\Bigg)$
	     >  $\Huge\frac{1}{\sigma_n^2}=\frac{n}{\sigma^2}+\frac{1}{\sigma_o^2},\quad\frac{\mu_n}{\sigma_n^2}=\frac{n}{\sigma^2}\hat{\mu_n}+\frac{\mu_0}{\sigma_0^2},\quad \hat{\mu_n}=\frac{1}{n}\sum\limits_{i=1}^{n}x_i$
