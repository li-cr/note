...
&=w^T(x-x_0) \quad [\;w=\Sigma^{-1}(\mu_i-\mu_j)\quad x_0=\frac{1}{2}(\mu_i+\mu_j)-s_{ij}(\mu_i-\mu_j) \quad s_{ij}=\frac{1}{(u_j-u_j)^T\Sigma^{-1}(u_i-u_j)}\ln\Big(\frac{P(\omega_i)}{P(\omega_j)}\Big)\;]
\end{aligned} 
}
$$
