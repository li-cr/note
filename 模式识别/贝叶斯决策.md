...
- ddd$\begin{aligned}x \sim N(\mu,\sigma^2) \implies p(x)=\frac{1}{\sqrt{2\pi \sigma} }\exp\left( -\frac{1}{2}\left(   \frac{x-\mu}{\sigma}  \right)^2 \right)\end{aligned}$ 
		-  在给定均值和方差的所有分布中，正态分布的熵最大
		-  实际环境中，很多类别的特征分布趋近正态分布
		-  根据Central Limit Theorem， 大量独立随机变量之和趋近于正态分布 ？（和还是均值）
</br>
</br>
- ddd$x \sim N(\mu,\sigma^2) \implies p(x)=\frac{1}{\sqrt{2\pi \sigma} }\exp\left( -\frac{1}{2}\left(   \frac{x-\mu}{\sigma}  \right)^2 \right)$ 
		-  在给定均值和方差的所有分布中，正态分布的熵最大
		-  实际环境中，很多类别的特征分布趋近正态分布
		-  根据Central Limit Theorem， 大量独立随机变量之和趋近于正态分布 ？（和还是均值）
...
w=\Sigma^{-1}(\mu_i-\mu_j) \\
x_0=\frac{1}{2}(\mu_i+\mu_j)-s_{ij}(\mu_i-\mu_j) \\
s_{ij}=\frac{1}{(u_j-u_j)^T\Sigma^{-1}(u_i-u_j)}\ln\Big(\frac{P(\omega_i)}{P(\omega_j)}\Big)
\end{cases} \\
