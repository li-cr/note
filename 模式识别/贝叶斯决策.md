...
&=\frac{(\mu_i-\mu_j)^T}{\sigma^2}\Big(x+ \sigma^2\frac{\ln(\frac{P(\omega_i)}{P(\omega_j)})}{(u_i-u_j)^T}-\frac{1}{2}\frac{\mu_i^T\mu_i-\mu_j^T\mu_j}{(\mu_i-\mu_j)^T} \Big)\\
&=\frac{(\mu_i-\mu_j)^T}{\sigma^2}\Big(x+ \sigma^2\frac{\ln(\frac{P(\omega_i)}{P(\omega_j)})}{||u_i-u_j||}(\mu_i-\mu_j)-\frac{1}{2}(\mu_i+\mu_j) \Big)\\
g(x_i)&\implies w^T(x-x_0)=0
\end{aligned}
}
$$
