...
g(x_i)-g(x_j) &=(u_i-u_j)^T\Sigma^{-1}\Big(x-\big(\frac{1}{2}(u_i+u_j)-\frac{u_i-u_j}{(u_j-u_j)^T\Sigma^{-1}(u_i-u_j)} \ln(\frac{P(\omega_i)}{P(\omega_j)})  \big) \Big) \\
&=w^T(x-x_0) \quad [\;w=\Sigma^{-1}(\mu_i-\mu_j)\quad x_0=\;\frac{1}{2}(u_i+u_j)-\frac{u_i-u_j}{(u_j-u_j)^T\Sigma^{-1}(u_i-u_j)} \ln(\frac{P(\omega_i)}{P(\omega_j)})]
\end{aligned} 
}
$$
